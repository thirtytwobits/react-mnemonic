...
- `src/` holds the library source. Public exports are in `src/index.ts` and the main module lives in `src/Mnemonic/` (`provider.tsx`, `use.ts`, `codecs.ts`, `types.ts`).
- Build outputs are emitted to `dist/` (published via `package.json` `files`).
...
- Keep exports centralized in `src/index.ts` and `src/index.ts`.
