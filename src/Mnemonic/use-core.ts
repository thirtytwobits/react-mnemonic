...
import type { UseMnemonicKeyOptions, ReconcileContext, MnemonicKeyState, MnemonicKeyDescriptor } from "./types";
...
function withReadMetadata<T>(value: T, rewriteRaw?: string): ReadResult<T> {
...
export function useMnemonicKey<T>(
    ...
    const applyReconcile = useCallback(
        ({
            ...
            if (!reconcile) {
                return withReadMetadata(value, rewriteRaw);
            }
            ...
            try {
                ...
                const nextRewriteRaw =
                    baselineSerialized === undefined || nextSerialized !== baselineSerialized
                        ? nextSerialized
                        : rewriteRaw;
                return withReadMetadata(reconciled, nextRewriteRaw);
            } catch (err) {
                const typedErr =
    ...
    useEffect(() => {
        ...
        warnOnce(
            ...
            `[Mnemonic] Conflicting useMnemonicKey contracts detected for key "${key}" in namespace "${api.prefix.slice(0, -1)}". Reuse a shared descriptor with defineMnemonicKey(...) or align defaultValue/codec/reconcile options so every consumer describes the same persisted contract.`,
        );
    }, [api, key, developmentRuntime, contractFingerprint, listenCrossTab, api.crossTabSyncMode, api.schemaRegistry]);
