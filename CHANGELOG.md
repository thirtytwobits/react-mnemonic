...
- A `listenCrossTab` hook no longer writes another tab's change back to storage. It stored each event's `newValue`, so when one tab wrote a key several times in quick succession, the other tab could write an earlier value after a later one had landed and leave storage holding the stale value; a late removal or `clear()` event likewise deleted a value written since. The hook now re-reads the key from storage when the event arrives
